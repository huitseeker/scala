...
trait ScalaSettings extends AbsScalaSettings
  ...
  val source        = ScalaVersionSetting("-source", "version", "Treat compiler input as Scala source of the specified version", ScalaVersion("2.11")) withPostSetHook ((version) => if (version.value < ScalaVersion("2.11")) {YrelaxAliasVarianceChecks.value = true})
  ...
  val YrelaxAliasVarianceChecks    = BooleanSetting   ("-Yrelax-variance-checks", "Lax variance interpretation for overridden members, see SI-6566.")
